...
# --- Page Configuration ---
st.set_page_config(layout="wide", page_title="MAYA AI: FIXED MASTER v35.14")

# --- Custom Styling (Dark & Bold) ---
st.markdown("""
    <style>
    ...
    /* Force Parallel History Table */
    .history-table { width: 100%; border: 2px solid #333; border-collapse: collapse; background: #fff; color: #000; table-layout: fixed; }
    .history-td { width: 50%; border: 1px solid #ccc; vertical-align: top; padding: 10px; word-wrap: break-word; }
...
# --- SIDEBAR (Fixed Variable Assignment) ---
uploaded_file = None # Initial definition to avoid NameError
with st.sidebar:
...
if uploaded_file:
    ...
    st.markdown("---")
    if st.button("🚀 LOAD TRIPLE AUDIT (v33 & v24 Parallel)"):
        for s_name in shifts:
            st.markdown(f"<div class='audit-title'>🎰 {s_name} Audit Card</div>", unsafe_allow_html=True)
            
            html_table = f"<table class='history-table'><tr><td class='history-td'><b>Engine v33</b><br>"
            ...
            html_table += "</td><td class='history-td'><b>Engine v24</b><br>"
